...
public sealed class MetricsExtensionsAndModelSelectorTests
{
    ...
    public async Task ToFriendlyString_Multiclass_IncludesAccuracyAndLogLoss()
    {
        ...
        var rows = Enumerable.Range(0, 12).Select(i => new IrisRow
        {
        ...
        var pipeline = ml.Transforms.Conversion.MapValueToKey("Label")
            ...
            .Append(ml.MulticlassClassification.Trainers.SdcaMaximumEntropy(new SdcaMaximumEntropyMulticlassTrainer.Options
            {
    ...
    public async Task ToFriendlyString_Regression_AndCrossValidation()
    {
        ...
        var rows = Enumerable.Range(1, 12).Select(i => new HouseRow { Label = i * 2f, Size = i, Rooms = i % 4 }).ToList();
        var data = ml.Data.LoadFromEnumerable(rows);
        var pipeline = ml.Transforms.Concatenate("Features", nameof(HouseRow.Size), nameof(HouseRow.Rooms))
            ...
            {
                LabelColumnName = nameof(HouseRow.Label),
                FeatureColumnName = "Features",
                MaximumNumberOfIterations = 1,
            }));
        var model = pipeline.Fit(data);
    ...
    [Test]
    public async Task ModelSelector_CanBeConstructed()
    {
    ...
    /// <summary>
    /// AutoML wall-clock experiment — opt-in only (default suite stays parallel/fast).
    /// </summary>
    [Test]
    [Explicit]
    public async Task ModelSelector_RunsRegressionExperiment_Smoke()
    {
        var rows = Enumerable.Range(1, 80).Select(i => new HouseRow { Label = i * 1.5f, Size = i, Rooms = i % 3 }).ToList();
        var selector = new ModelSelector<HouseRow>();
        var result = selector.RunRegressionExperiment(rows, nameof(HouseRow.Label), runtime: 10);
